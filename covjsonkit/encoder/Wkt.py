...
class Wkt(Encoder):
        ...
        for date in coords.keys():
            for dt in coords[date]["t"]:
                ...
                for level in levels:
                    for cor in coord:
                        if include_z:
                            coordinates[dt]["composite"].append([cor[1], cor[0], level])
                        else:
                            coordinates[dt]["composite"].append([cor[1], cor[0]])
    ...
    def from_polytope_month(self, result):
        ...
        # Build composite coordinates: expand spatial points across all levels per date.
        coordinates = {}
        for date in fields["dates"]:
